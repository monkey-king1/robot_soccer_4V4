...
bool is_getball_s(const point2f& ball, const point2f& runner, const float& dir){
	//miss参数需要在比赛时实地调试
...
void superforward(const WorldModel* model, int robot_id, PlayerTask& task){
	...
	if (is_getball_s(ball, runner, dir)){
